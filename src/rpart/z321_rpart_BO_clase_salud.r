...
hs  <- makeParamSet(
          ...
          makeIntegerParam("minsplit" , lower=  1L  , upper= 1600L),  #la letra L al final significa ENTERO
          makeIntegerParam("minbucket", lower=  1L  , upper= 500L),
          makeIntegerParam("maxdepth" , lower=  8L  , upper=   10L),
...
# particionar( data=dataset, division=c(70,30), agrupa=clase_salud, seed=semilla)   crea una particion 70, 30 
# particionar( data=dataset, division=c(1,1,1,1,1), agrupa=clase_salud, seed=semilla)   divide el dataset en 5 particiones
...
#cargo los datos
dataset  <- fread("./datasets/dataset_pequeno_clase_salud.csv")
...
archivo_log  <- "HB321.txt"
archivo_BO   <- "HB321.RDATA"
